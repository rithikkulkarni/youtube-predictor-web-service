...
def get_pipeline_for(group: str):
    """
    Given a group name (e.g. "group1"), lazily load models/group1_rf.pkl
    and cache it. Raises FileNotFoundError if missing.
    """
...
async def predict_group(
    ...
    # 1) lazy load the pipeline for this group
    try:
        pipeline = get_pipeline_for(group)
    except FileNotFoundError:
        raise HTTPException(
            status_code=404,
            detail=f"Unknown group '{group}'. models/{group}_rf.pkl not found."
        )

    # 2) preprocess uploaded thumbnail
    ...
    # 5) assemble in model‑expected order
    fv = {
        "avg_red": avg_red,
        "avg_green": avg_green,
        "avg_blue": avg_blue,
